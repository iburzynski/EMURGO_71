...
groupStudents' g [] = g
groupStudents' g (sr:srs) = case group of
  'A' -> groupStudents' (name : as, bs, cs) srs
  'B' -> groupStudents' (as, name : bs, cs) srs
  'C' -> groupStudents' (as, bs, name : cs) srs
  _   -> groupStudents' g srs
  where
    (name, group) = sr
    (as, bs, cs) = g
