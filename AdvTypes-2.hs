...
--   * Show: convert values of a type to a string
--   * Read: convert a string to a value of a type (use discouraged - use parsers instead)
...
-- *** Further reading: Get Programming with Haskell by Will Kurt: Lessons 16, 18, 19

-- *** Correction in Advanced Types Assessment:
-- Q: "How many values of `Color` are there if `data Color = Color Int Int Int`
--     and each Int field can be one of 256 possible values?"